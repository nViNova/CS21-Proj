...
def assembler(instr: str, form: str) -> str:  # has to be a str, need to sometimes prepend 0s to hex or bin
    ...
    if len(instr) == 1:
        ...
        converted_instr = int(INST[instr[0]], 16)  # turn str to hex to convert properly to bin later
        if form == 'bin':
        ...
        if inst in START_END_INST:
            ...
            converted_instr = f'{int(start, 16):04b}' + REG[reg] + end  # end is only 1 bit can put it as is
            ...
            if len(hex_imm) > 1:
                raise Exception("Invalid Immediate, Must be at most 4 bits")

            # combine inst in hex with imm in hex
            converted_instr = INST[inst] + hex_imm

            if form == 'hex':
                return '0' + converted_instr[2:]  # remove 0x and append a 0 at the start
...
# test insrt onlies
q = assembler('rot-rc', 'bin')
print(q)
q = assembler('rot-rc', 'hex')
print(q)

# test reg 
q = assembler('inc*-reg RC', 'bin')
print(q)
q = assembler('inc*-reg RC', 'hex')
print(q)
q = assembler('to-reg RC', 'bin')
print(q)
q = assembler('to-reg RC', 'hex')
print(q)

# test 8 wide imm

# test 16 wide imm
q = assembler('r4 15', 'bin')
print(q)
q = assembler('r4 15', 'hex')
print(q)

q = assembler('acc 15', 'bin')
print(q)
q = assembler('acc 15', 'hex')
print(q)

q = assembler('b-bit 2 15', 'bin')
print(q)
q = assembler('b-bit 2 15', 'hex')
print(q)

q = assembler('bnz-a 15', 'bin')
print(q)
q = assembler('bnz-a 15', 'hex')
print(q)

q = assembler('bnez-cf 156', 'bin')
print(q)
q = assembler('bnez-cf 156', 'hex')
print(q)

q = assembler('b 156', 'bin')
print(q)
q = assembler('b 156', 'hex')
print(q)

q = assembler('call 156', 'bin')
print(q)
q = assembler('call 156', 'hex')
print(q)